...
def start_keyboard_teleop(controller: Arx5CartesianController):
    ...
    target_gripper_pos = float(np.clip(controller.get_eef_state().gripper_pos, 0.0, controller.get_robot_config().gripper_width))
    cmd_dt = 0.01
    preview_time = 0.1
    window_size = 5
    ...
    while True:
        ...
        if key_space:
            ...
            target_pose_6d = controller.get_home_pose()
            target_gripper_pos = float(np.clip(controller.get_eef_state().gripper_pos, 0.0, robot_config.gripper_width))
            loop_cnt = 0
            start_time = time.monotonic()
            continue
